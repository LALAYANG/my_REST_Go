# to replace REST_Go/run_service.py 
...
def run_service(service_path, class_name):
    ...
    with open(service_path + "/run.sh", 'w') as f:
        f.write("java -Djdk.attach.allowAttachSelf=true " + cov + " -cp target/classes:target/test-classes:" + cp + ' ' + class_name + " > " + log_name)
...
if __name__ == "__main__":
    ...
    evo = sys.argv[3]
    log_dir = sys.argv[4]
    log_name = os.path.join(log_dir,"log_"+ cov_port + "_" + name + "_" + evo+".txt")
    base = os.getcwd()
    print("start to run service ", name)
    ...
    if evo == "whitebox":
        ...
        if name == "features-service":
            ...
            # "/home/azureuser/EMB/jdk_8_maven/em/embedded/rest/ocvn",
            ...
            # run_service("./services/evo_jdk8/cs/rest/artificial/scs", "cs.rest.org.restscs.RunServer")
            ...
            subprocess.run("tmux new -d -s erc20-rest-service '. java8.env && java " + cov + " -jar ./services/jdk8/erc20-rest-service/build/libs/erc20-rest-service-0.1.0.jar" + " > " + log_name + "'", shell=True)
        elif name == "genome-nexus":
            subprocess.run("sudo docker run --name=gn-mongo --restart=always -p 27018:27017 -d genomenexus/gn-mongo:latest", shell=True)
            time.sleep(30)
            subprocess.run("tmux new -d -s genome-nexus '. java8.env && java " + cov + " -jar ./services/jdk8/genome-nexus/web/target/web-0-unknown-version-SNAPSHOT.war" + " > " + log_name + "'", shell=True)
        elif name == "person-controller":
            subprocess.run("sudo docker run -d -p 27019:27017 --name mongodb mongo:latest", shell=True)
            time.sleep(30)
            subprocess.run("tmux new -d -s person-controller '. java8.env && java " + cov + " -jar ./services/jdk8/person-controller/target/java-spring-boot-mongodb-starter-1.0.0.jar" + " > " + log_name + "'", shell=True)
        elif name == "problem-controller":
            subprocess.run("sudo docker run -d -p 3307:3306 --name mysql -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=test mysql", shell=True)
            time.sleep(30)
            subprocess.run("tmux new -d -s problem-controller '. java8.env && java " + cov + " -jar ./services/jdk8/problem-controller/target/project-api-0.0.1.jar" + " > " + log_name + "'", shell=True)
        elif name == "rest-study":
            subprocess.run("tmux new -d -s rest-study '. java8.env && java " + cov + " -jar ./services/jdk8/rest-study/target/restful-web-services-0.0.1-SNAPSHOT.jar" + " > " + log_name + "'", shell=True)
        elif name == "spring-batch-rest":
            subprocess.run("tmux new -d -s spring-batch-rest '. java8.env && java " + cov + " -jar ./services/jdk8/spring-batch-rest/example/api/target/spring-batch-rest-example-core-1.5.1.jar" + " > " + log_name + "'", shell=True)
        elif name == "spring-boot-sample-app":
            run_service("./services/jdk8/spring-boot-sample-app", "com.test.sampleapp.Application")
        elif name == "user-management":
            subprocess.run("sudo docker run -d -p 3306:3306 --name mysqldb -e MYSQL_ROOT_PASSWORD=root -e MYSQL_DATABASE=users mysql", shell=True)
            time.sleep(30)
            subprocess.run("tmux new -d -s user-management '. java8.env && java " + cov + " -jar ./services/jdk8/user-management/target/microdemo2-1.0.0-SNAPSHOT.jar" + " > " + log_name + "'", shell=True)
        elif name == "cwa-verification":
            ...
            subprocess.run("tmux new -d -s market '. java11.env && java -Djdk.attach.allowAttachSelf=true " + cov + " -jar ./services/jdk11/market/market-rest/target/market-rest-0.1.2.jar" + " > " + log_name + "'", shell=True)
        elif name == "project-tracking-system":
            subprocess.run("tmux new -d -s project-tracking-system '. java11.env && java -Djdk.attach.allowAttachSelf=true " + cov + " -jar ./services/jdk11/project-tracking-system/target/project-tracking-system.jar" + " > " + log_name + "'", shell=True)
